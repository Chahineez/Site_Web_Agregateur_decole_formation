function fetchdata(){
    $.ajax({
  type: 'post',
  url: '../Controller/readTypeFormations.php',
  dataType: 'json',
  cache :false,
  success: function (arr) {
    ...
    for(var x=0 ; x < arr.length ; x++){
                ...
                var th = $ ("<th scope='row'>" + nom +"</th>");
                var td1 = $ ("<td>" + heure +" H</td>");
                var td2 = $ ("<td>"+ ht +" DA</td>");
                var td3 = $ ("<td>"+ taux +"</td>");
                var td4 = $ ("<td>"+ ttc+" DA</td>");
                var td4 = $ ("<td>"+ ttc+" DA</td>");
                var td5 = $('<td><input id ="'+id+'" class="supp" type="button" value="supprimer"></td>');
